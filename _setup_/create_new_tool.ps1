...
# renames all the instances of the current ExceptionDialog with the prompted new name
# searches through files and replaces in there as well

# parent directory of the exception-dialog
$TOOLS_DIR = (Get-Item -Path "..\").FullName

# define current tool name variables
$CURRENT_base_case = "exception-dialog"
$CURRENT_snake_case = "exception_dialog"
$CURRENT_PascalCase = "ExceptionDialog"
...
# Find instances of exception_dialog in files and replace them with the new tool name
...
Foreach-Object {
    ...
    ((Get-Content -path $_.FullName -Raw) -replace "exception-dialog", $NEW_base_case) | Set-Content -Path $_.FullName
